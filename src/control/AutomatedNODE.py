...
class AutomatedControlTransmitterProcess():
    ...
    def __init__(self):
        ...
        time.sleep(1)
        command = {'action': '1', 'speed': 25.0}
        self.sendCommand(command)
        
        # command = {'action': '1', 'speed': 20.0}
        # self.sendCommand(command)
    ...
    def callback_xmid(self, data):
        ...
        self.steer, self.speed = self.PIDfunction(data['x'])
        print(self.steer)
    ...
    def PIDfunction(self, position):
        ...
        speed = 40 - abs(self.PID)
        
        return self.PID, speed
    ...
    def callback_main(self, data):
        ...
        if self.order == 4:
            ...
            #front_command = {'action': '2', 'steerAngle': self.steer}
                self.sendCommand(command)
                #time.sleep(0.001)
                self.order = 2
                #print(self.order)
            ...
            #command = {'action': '4', 'activate': True}
            #self.sendCommand(command)side = self.dis[5]-self.dis[2]
